...
def calculate_psnr(img1, img2, border=0):
    ...
    h, w, c = img1.shape
    img1 = img1[border:h-border, border:w-border, -50:]
    img2 = img2[border:h-border, border:w-border, -50:]
...
def calculate_pcc(img1, img2):
    ...
    pcc_values = np.zeros(num_channels)
    for c in range(50):
        img1_channel = img1[:, :, num_channels-50+c].flatten()
        img2_channel = img2[:, :, num_channels-50+c].flatten()
        pcc_values[c] = pearsonr(img1_channel, img2_channel)[0]
    
    pcc_cell = pearsonr(img1[:,:,7].flatten(), img2[:,:,7].flatten())[0]

    return np.nanmean(pcc_values), pcc_cell
...
def calculate_ssim(img1, img2, border=0):
    ...
    h, w, c = img1.shape
    if c < 50:
        raise ValueError('The height of the images must be at least 50.')
    img1 = img1[border:h-border, border:w-border, -50:]
    img2 = img2[border:h-border, border:w-border, -50:]
